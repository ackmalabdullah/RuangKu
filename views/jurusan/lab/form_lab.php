...
$id_lab = $_GET['id'] ?? null; // Ganti id_ruangan menjadi id_lab
$lab_data = [ // Ganti ruangan_data menjadi lab_data
    'nama_lab' => '', // Ganti nama_ruangan menjadi nama_lab
    'kategori_id' => '',
    'lokasi' => '',
    'kapasitas' => 0,
    'status_lab' => 'tersedia', // Ganti status_ruangan menjadi status_lab
    'gambar' => ''
];
$fasilitas_lab = []; // Ganti fasilitas_ruangan menjadi fasilitas_lab

// --- 1. AMBIL DATA KATEGORI (Dropdown) ---
// Tabel: kategori_ruangan (Sesuai skema)
...
if ($id_lab) {
    // A. Ambil data utama laboratorium
    // Tabel: laboratorium
    $query_lab = "SELECT * FROM laboratorium WHERE id_lab = '" . mysqli_real_escape_string($koneksi, $id_lab) . "'";
    ...
    if (mysqli_num_rows($result_lab) == 0) {
        ...
        header('Location: ruangan.php'); // Dibiarkan ruangan.php atau ganti lab.php jika ada
        exit;
    }
    $lab_data = mysqli_fetch_assoc($result_lab); // Ganti ruangan_data menjadi lab_data

    // B. Ambil data relasi fasilitas laboratorium
    // Tabel: lab_fasilitas
    $query_fasilitas_relasi = "
        ...
        FROM 
            lab_fasilitas 
        WHERE 
            lab_id = '" . mysqli_real_escape_string($koneksi, $id_lab) . "'"; // Ganti ruangan_id menjadi lab_id
    ...
    while ($relasi = mysqli_fetch_assoc($result_fasilitas_relasi)) {
        $fasilitas_lab[$relasi['fasilitas_id']] = $relasi['jumlah']; // Ganti fasilitas_ruangan menjadi fasilitas_lab
    }
}

$form_title = $id_lab ? "Edit Data Laboratorium: " . htmlspecialchars($lab_data['nama_lab']) : "Tambah Data Laboratorium Baru"; // Ganti Ruangan menjadi Laboratorium
?>

<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Laboratorium /</span> <?= $form_title; ?></h4>

    <div class="row">
        <div class="col-xl-12">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Form Laboratorium</h5>
                </div>
                <div class="card-body">
                    
                    <form action="proses_lab.php" method="POST" enctype="multipart/form-data">
                        <?php if ($id_lab): ?>
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="nama_lab">Nama Laboratorium</label> <input type="text" 
                                class="form-control" 
                                id="nama_lab" 
                                name="nama_lab" 
                                value="<?= htmlspecialchars($lab_data['nama_lab']); ?>" 
                                required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="kategori_id" name="kategori_id" class="form-select" required>
                                ...
                                while ($kategori = mysqli_fetch_assoc($result_kategori)): 
                                    $selected = ($lab_data['kategori_id'] == $kategori['id_kategori']) ? 'selected' : '';
                                ?>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                ...
                                id="lokasi" 
                                name="lokasi" 
                                value="<?= htmlspecialchars($lab_data['lokasi']); ?>" 
                                required>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" 
                                    ...
                                    id="kapasitas" 
                                    name="kapasitas" 
                                    value="<?= htmlspecialchars($lab_data['kapasitas']); ?>" 
                                    min="1"
                                    required>
                            </div>

                            <div class="col-md-6 mb-3">
                                <label class="form-label" for="status_lab">Status Laboratorium</label> <select id="status_lab" name="status_lab" class="form-select" required>
                                    <?php 
                                    // Status sesuai ENUM di tabel laboratorium: 'tersedia', 'dalam_perbaikan', 'tidak_tersedia'
                                    $statuses = ['tersedia', 'dalam_perbaikan', 'tidak_tersedia'];
                                    foreach ($statuses as $status): 
                                        $display = str_replace('_', ' ', ucwords($status));
                                        $selected = ($lab_data['status_lab'] == $status) ? 'selected' : '';
                                    ?>
                        ...
                        <div class="mb-3">
                            <label for="gambar" class="form-label">Gambar/Foto Laboratorium</label> <input class="form-control" type="file" id="gambar" name="gambar" <?= $id_lab ? '' : 'required'; ?>>
                            <?php if ($lab_data['gambar']): ?>
                                <small class="text-muted mt-2 d-block">Gambar saat ini: <?= htmlspecialchars($lab_data['gambar']); ?></small>
                                <input type="hidden" name="gambar_lama" value="<?= htmlspecialchars($lab_data['gambar']); ?>">
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="row mb-3 border p-3 rounded">
                            ...
                            while ($fasilitas = mysqli_fetch_assoc($result_fasilitas_list)):
                                ...
                                $is_checked = isset($fasilitas_lab[$fasilitas_id]); // Cek dari fasilitas_lab
                                $jumlah_value = $is_checked ? $fasilitas_lab[$fasilitas_id] : 1;
                            ?>
                            <div class="col-md-6 mb-3">
                        ...
                        <a href="lab.php" class="btn btn-outline-secondary">Batal</a>
                    </form>
